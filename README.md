...
- 酱油鸡：全局最多 3 次，第 5/6 轮合计最多 1 次；先正常提交并声明使用技能，等所有玩家提交后，系统只向酱油鸡展示各城总兵力分布，然后酱油鸡必须把自己 1 个兵从一个城移动到另一个城。
...
- 当前版本房间数据保存在服务端内存里，服务器重启、Render 免费实例睡眠后重新唤醒、重新部署后房间会消失。
- 页面在房间内时会每 15 秒发送一次心跳，并在 WebSocket 重连后自动恢复玩家身份，尽量让 Render 免费实例在有人在线时保持活跃；但免费实例仍不保证永久运行。
